...
/**
 * @param langNamesMapping {{codes: Record<string, {name: string, isGroup?: boolean, isSpecial?: boolean}>, redirects: Record<string, string>}}
 */
function init(langNamesMapping) {
  ...
  for (const [code, langData] of Object.entries(langNamesMapping.codes)) {
    if (langData.isGroup || langData.isSpecial) continue;
    LANGUAGES.set(code, langData.name);
  }
  // Flatten redirects
  for (const [code1, code2] of Object.entries(langNamesMapping.redirects)) {
    const langName = LANGUAGES.get(code2);
    if (langName) LANGUAGES.set(code1, langName);
  }
